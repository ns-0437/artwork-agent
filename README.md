...
**Day 5 (evaluation harness): first frozen results.** 34 fixtures (16 dev, 18 held-out) driven through the real stack end to end in three modes - agent (Groq), a genuinely agent-disabled baseline, and a scripted (rules + deterministic script, no LLM) comparison. Agent: 34/34 passed, 0 timed out, 0 falsely-resolved held-out cases; every repair's trim region and every proof asset independently re-verified from the storage backend, not just trusted from DB metadata. Rules alone leave 8/34 cases silently stuck; having either the agent or a script resolves 18/34 - the agent's distinct value in this pass is escalating 6 cases a script would otherwise leave `BLOCKED` forever, not resolving different cases than a script would. Four held-out fixtures are deliberately withheld from this pass for a genuinely blind follow-up (see `evals/CHANGES.md`, which also discloses one fixture whose expectation was adjusted after observing model behavior). See docs/architecture.md's "Day 5: evaluation harness and first results" for the full numbers and caveats. GCS storage backend and Cloud Run deployment configs are prepared (`infra/gcp/`) but not executed against a live project.
