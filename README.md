...
Deployed revision: commit [`ead996b`](https://github.com/ns-0437/artwork-agent/commit/ead996b63239c15daa2b47052bef8a6eda8cb457) (`master`, 2026-09-16) - `artwork-agent-api-00004-cmr`, `artwork-agent-worker-00005-9c5`, `artwork-agent-image-python-00001-v5w` on Cloud Run. Verified against the live deployment, not just locally: an unauthenticated request to `image-python` returns `403`; the full upload -> inspect -> repair -> proof loop was driven end to end through the deployed frontend and reached `RESOLVED` / `AWAITING_CUSTOMER_APPROVAL`. The provider-error-vs-model-escalation distinction (see below) was verified separately, against a deliberately broken key on the local stack, not the live deployment - see `evals/scripts/run_eval.py`'s `no_provider_error` check.
