...
def test_extends_canvas_by_the_rounded_up_margin():
    ...
    result = extend_background(img, "RGB", (10, 20, 30), effective_ppi=300.0)
...
def test_pastes_original_unresampled_at_the_trim_offset():
    ...
    result = extend_background(img, "RGB", (200, 100, 50), effective_ppi=300.0)
    canvas = result["image"]
...
def test_new_margin_is_filled_with_the_edge_color():
    ...
    result = extend_background(img, "RGB", (5, 5, 5), effective_ppi=300.0)
    canvas = result["image"]
    assert canvas.getpixel((0, 0)) == (5, 5, 5)
    assert canvas.getpixel((canvas.width - 1, canvas.height - 1)) == (5, 5, 5)


def test_preserves_grayscale_mode_rather_than_forcing_rgb():
    img = Image.new("L", (60, 60), 200)
    result = extend_background(img, "L", 200, effective_ppi=300.0)
    canvas = result["image"]
    assert canvas.mode == "L"
    assert canvas.getpixel((0, 0)) == 200


def test_abnormally_large_effective_ppi_is_rejected_before_allocating():
    img = Image.new("RGB", (900, 900), (10, 20, 30))
    # A huge effective_ppi (as from a tiny declared physical size) would
    # otherwise drive an enormous canvas allocation.
    with pytest.raises(CanvasTooLargeError):
        extend_background(img, "RGB", (10, 20, 30), effective_ppi=1_000_000.0)
